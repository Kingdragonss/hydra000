...
exports.run = async (client, message, args) => {
  ...
  if (
    !message.member.roles.cache.get(ayarlar.MuteYetkilisi) &&
    !message.member.hasPermission("741973537669251163")
  )
  ...
  if (!muted) {
    message.channel.send(`Mute Atmam Gereken Kişiyi Belirt.`);
  } else {
    if (muted.roles.highest.position >= message.member.roles.highest.position) {
      ...
      if (!mutezaman) {
        ...
        db.set(
          `süre_${message.mentions.users.first().id + message.guild.id}`,
          mutezaman
        );
...
• Sebep: \`${sebep}\``)
        ...
        setTimeout(async function() {
          muted.roles.remove(
            ayarlar.MuteliRol,
            client.channels.cache.get(ayarlar.MuteKanal).send(
              new Discord.MessageEmbed().setColor("#494459")
