...
module.exports.run = async (client, message, args) => {
  ...
  if (
    !message.member.roles.cache.has(ayarlar.JailYetkilisi) &&
  ...
  const kişi = message.guild.member(
  ...
  if (kişi.roles.highest.position >= message.member.roles.highest.position)
    return message.channel.send(
      "Etiketlenen Kullanıcı Sizden Üst/Aynı Pozisyonda."
    );
  ...
  db.set(
    `süreJail_${message.mentions.users.first().id + message.guild.id}`,
    zaman1
  );
  ...
  const jail = new Discord.MessageEmbed()
    ...
    .setDescription(
      `<@${kişi.id}> (\`${kişi.id}\`) üyesi sunucuda cezalandırıldı.
